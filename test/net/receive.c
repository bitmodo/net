...
ParameterizedTestParameters(method, receive) {
    const size_t size = 3;
...
ParameterizedTest(Params * param, method, receive) {
    cr_log_info("Running receive test with {handler=%p, useMethod=%d, returnValue=%d}\n",
            param->handler, param->useMethod, param->returnValue);

    gHandler = param->handler;
    if (gHandler && param->useMethod) {
        gHandler->receive = &customFunction;
    }

    cr_assert_eq(netReceive(NULL, NULL, 0, 0), param->returnValue, "Did not get the expected return value");
}
