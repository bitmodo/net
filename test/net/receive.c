...
int customReceive(Socket * sock, void * buf, int size) {
    receive = true;
    hasSocket = sock != NULL;
...
int main() {
    ...
    sock = NULL;
    buf = NULL;
    assert(netReceive(sock, buf, sizeof(void)) == ENULL_POINTER && "Receive with a null handler and socket did not return a null pointer error");

    gHandler = malloc(sizeof(NetHandler));
    *gHandler = (NetHandler) {NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL};
    sock = NULL;
    buf = NULL;
    assert(netReceive(sock, buf, sizeof(void)) == ENULL_POINTER && "Receive with a null socket and function did not return a null pointer error");
    free(gHandler);
    ...
    *gHandler = (NetHandler) {NULL, NULL, NULL, NULL, &customReceive, NULL, NULL, NULL, NULL, NULL};
    sock = NULL;
    buf = NULL;
    assert(netReceive(sock, buf, sizeof(void)) == RECEIVE_RETURN && "Receive did not return the correct return value");
    ...
    *gHandler = (NetHandler) {NULL, NULL, NULL, NULL, &customReceive, NULL, NULL, NULL, NULL, NULL};
    ...
    assert(netReceive(sock, buf, sizeof(int)) == RECEIVE_RETURN && "Receive did not return the correct return value");
    assert(receive && "Receive did not set the expected variable");
    assert(hasSocket && "Has socket was set to false with a socket");
