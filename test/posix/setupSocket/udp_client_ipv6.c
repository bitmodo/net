...
int getaddrinfo(const char * name, const char * service, const struct addrinfo * req, struct addrinfo ** pai) {
    ...
    assert(req->ai_family == AF_INET6 && "Did not get expected address family");
    assert(req->ai_socktype == SOCK_DGRAM && "Did not get expected socket type");
    assert((req->ai_flags & (AI_V4MAPPED | AI_ADDRCONFIG)) && "Did not get expected socket flags");
    assert(req->ai_protocol == IPPROTO_UDP && "Did not get expected protocol");
...
int main() {
    ...
    *sock = (Socket) {.side = CLIENT, .type = UDP, .addressType = IPv6};
