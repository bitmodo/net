...
int connectLinux(Socket * sock) {
    ...
    if (sock->address) {
        if (inet_pton(AF_INET, sock->address, &(addr.sin_addr)) <= 0) {
            return EINVALID_IP;
        }
    } else {
        return ENULL_POINTER;
    ...
    if (connect(sock->data->fd, (struct sockaddr *) &addr, sizeof(struct sockaddr_in)) < 0) {
        return EUNKNOWN;
    }
...
int startLinux(Socket * sock) {
    ...
    if (bind(sock->data->fd, (struct sockaddr *) &addr, sizeof(struct sockaddr_in)) < 0) {
        return EUNKNOWN;
    }

    if (listen(sock->data->fd, 10)) {
        return EUNKNOWN;
    }
...
int loopLinux(Socket * sock) {
    ...
    int fd;
    if ((fd = accept(sock->data->fd, NULL, 0)) < 0) {
        return EUNKNOWN;
    }
...
int receiveLinux(Socket * sock, void * buf, int count) {
    if (!sock || !(sock->data) || (sock->side == SERVER && sock->data->conn == -1)) {
        return ENULL_POINTER;
    }

    int size;
    if ((size = recv(sock->side == SERVER ? sock->data->conn : sock->data->fd, buf, count, 0)) < 0) {
        return EUNKNOWN;
    }
...
int sendLinux(Socket * sock, const void * buf, int count) {
    ...
    if (send(sock->side == SERVER ? sock->data->conn : sock->data->fd, buf, count, 0) < 0) {
        return EUNKNOWN;
    }
